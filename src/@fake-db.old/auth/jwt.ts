...
mock.onPost('/jwt/register').reply(request => {
  if (request.data.length > 0) {
    ...
    const error = {
      email: isEmailAlreadyInUse ? 'Este e-mail já está em uso.' : null,
      username: isUsernameAlreadyInUse ? 'Este usuário já está em uso.' : null
    }
...
mock.onGet('/auth/me').reply(config => {
  ...
  if (decoded) {
    ...
    return [401, { error: { error: 'Usuário inválido' } }]
  }
})
