...
const EditActions = ({ id, toggleSendOrderDrawer, toggleAddPaymentDrawer }: Props) => {
  return (
    <Box>
      <Card sx={{ mb: 4 }}>
        <CardContent>
          ...
          <Link href={`/manage/order/preview/${id}`} passHref>
      ...
      <OptionsWrapper sx={{ mb: 1 }}>
        <InputLabel
          htmlFor='order-edit-payment-terms'
          sx={{ cursor: 'pointer', fontSize: '0.875rem', color: 'text.secondary' }}
        >
          Payment Terms
        </InputLabel>
        <Switch defaultChecked id='order-edit-payment-terms' />
      </OptionsWrapper>
      <OptionsWrapper sx={{ mb: 1 }}>
        <InputLabel
          htmlFor='order-edit-client-notes'
          sx={{ cursor: 'pointer', fontSize: '0.875rem', color: 'text.secondary' }}
        >
          Client Notes
        </InputLabel>
        <Switch id='order-edit-client-notes' />
      </OptionsWrapper>
      <OptionsWrapper>
        <InputLabel
          htmlFor='order-edit-payment-stub'
          sx={{ cursor: 'pointer', fontSize: '0.875rem', color: 'text.secondary' }}
        >
          Payment Stub
        </InputLabel>
        <Switch id='order-edit-payment-stub' />
      </OptionsWrapper>
    </Box>
  )
}
