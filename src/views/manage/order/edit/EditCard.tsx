...
import { SingleOrderType, OrderClientType } from 'src/types/manage/orderTypes'
...
const EditCard = ({ data }: Props) => {
  ...
  useEffect(() => {
    axios.get('/manage/order/clients').then(response => {
  ...
  if (data) {
    return (
      <Card>
        ...
        <CardContent sx={{ pt: 4 }}>
          <InputLabel htmlFor='order-note' sx={{ mb: 2 }}>
            Note:
          </InputLabel>
          <TextField
            rows={2}
