...
// ** Types
import { SingleOrderType } from 'src/types/manage/orderTypes'
      ...
      <Card>
                  ...
                  <Table sx={{ maxWidth: '200px' }}>
                    <TableBody>
                      ...
                      <TableRow>
                        ...
                        <MUITableCell>
                          <Typography variant='body2'>{data.order.dueDate}</Typography>
                        </MUITableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
        ...
        <CardContent>
          <Box sx={{ mt: 0, width: '100%', display: 'flex', justifyContent: 'flex-end' }}>
            <Link href={`/manage/order/print/${data.order.id}`} passHref>
              <Button sx={{ mr: 4 }} target='_blank' component='a' variant='contained'>
                Print
              </Button>
            </Link>

            <ReactToPdf scale={0.845} targetRef={PreviewRef} filename={`order-${data.order.id}.pdf`}>
              {({ toPdf }: { toPdf: () => void }) => {
                return (
