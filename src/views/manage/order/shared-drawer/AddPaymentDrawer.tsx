...
const EditOrderDrawer = ({ open, toggle }: Props) => {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: 5 }}>
        <Box sx={{ mb: 6 }}>
          <InputLabel htmlFor='order-balance'>Order Balance</InputLabel>
          <TextField fullWidth id='order-balance' InputProps={{ disabled: true }} defaultValue='5000.00' />
        </Box>
        <Box sx={{ mb: 6 }}>
          <CleaveWrapper>
            <InputLabel htmlFor='order-payment-amt'>Payment Amount</InputLabel>
            <Cleave
              id='order-payment-amt'
              options={{ prefix: '$', numeral: true, numeralThousandsGroupStyle: 'thousand' }}
            />
          </CleaveWrapper>
        </Box>
        <Box sx={{ mb: 6 }}>
          <InputLabel htmlFor='order-payment-date'>Payment Date</InputLabel>
