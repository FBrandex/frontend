  ...
  return (
    <Fragment>
      ...
      >
        ...
        <Link href={`/manage/orders/edit/${id}`} passHref>
          <MenuItem>
...
const renderClient = (row: InvoiceType) => {
  if (row.avatar.length) {
    ...
    return (
      ...
      >
        {getInitials(row.client || 'John Doe')}
      </CustomAvatar>
    )
  }
}
...
const defaultColumns = [
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => (
      <Link href={`/manage/orders/preview/${row.invoice}`} passHref>
        <StyledLink>{`${row.invoice}`}</StyledLink>
      </Link>
    )
  },
  /* {
  ...
  },
  {
    ...
    field: 'total',
    headerName: 'Valor',
    renderCell: ({ row }: CellType) => <Typography variant='body2'>{`R$ ${row.total || 0}`}</Typography>
  },
  /* {
    flex: 0.1,
    minWidth: 90,
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }: CellType) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Tooltip title='Ver'>
            <Box>
              <Link href={`/manage/orders/preview/${row.id}`} passHref>
